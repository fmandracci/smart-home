 ...
 */
bool modbus::modbus_connect(std::string host, uint16_t port=502) {
    HOST = host;
    PORT = port;
 ...
 */
int8_t modbus::modbus_write(int address, uint amount, int func, const uint16_t *value)
{
    int8_t status = 0;
 ...
 * @param func      Modbus Functional Code
 */
int8_t modbus::modbus_read(int address, uint amount, int func)
{
    uint8_t to_send[12];
    modbus_build_request(to_send, address, func);
    to_send[5] = 6;
 ...
 */
int8_t modbus::modbus_write_coil(int address, const bool& to_write)
{
    if(_connected) {
        if(address > 65535) {
            set_bad_input();
 ...
 */
int8_t modbus::modbus_write_register(int address, const uint16_t& value)
{
    if(_connected) {
        if(address > 65535) {
            set_bad_input();
 ...
 */
int8_t modbus::modbus_write_coils(int address, int amount, const bool *value)
{
    if(_connected) {
 ...
 */
int8_t modbus::modbus_write_registers(int address, int amount, const uint16_t *value)
{
    if(_connected) {
        if(address > 65535 || amount > 65535) {
            set_bad_input();
 ...
 */
int8_t modbus::modbus_send(uint8_t *to_send, int length) {
    _msg_id++;
    return ((int8_t) send(_socket, to_send, (size_t)length, 0));
}
 ...
 */
int modbus::modbus_receive(uint8_t *buffer) const {
    return ((int) recv(_socket, (char *) buffer, 1024, 0));
}
