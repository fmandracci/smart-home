...
private:
    ...
    int8_t modbus_read(int address, uint amount, int func);
    int8_t modbus_write(int address, uint amount, int func, const uint16_t *value);

//    inline ssize_t modbus_send(uint8_t *to_send, int length);
//    inline ssize_t modbus_receive(uint8_t *buffer) const;

    int8_t modbus_send(uint8_t *to_send, int length);
    int modbus_receive(uint8_t *buffer) const;
...
public:
    ...
    bool modbus_connect(std::string host, uint16_t port);
    ...
    int8_t modbus_write_coil(int address, const bool& to_write);
    int8_t modbus_write_register(int address, const uint16_t& value);
    int8_t modbus_write_coils(int address, int amount, const bool *value);
    int8_t modbus_write_registers(int address, int amount, const uint16_t *value);
